...
int player_play(player_t player)
{
    ...
    if (player->status != PLAYER_RUNNING)
    {
        ...
        /* 打开全局中断 */
        cpu_irq_restore(level);

        /*释放信号量通知播放线程开始播放*/
        sem_give(&player->sem_play);
    }
...
int player_show(player_t player)
{
    ...
    {
			trace_printf("<---  Playing %02d: ", player->song_current);
    }
    else
    {
			trace_printf("<---   Paused %02d: ", player->song_current);
    }
    ...
    trace_printf("Progress : %02d%%  Volume: %02d%%\n", percent, player->volume);
...
static void player_entry(void *parameter)
{
    ...
    while (1)
    {
        ...
        else
        {
            ...
            /* 等待播放的信号量 */
            sem_take(&player->sem_play, WAIT_FOREVER);
...
int player_start(player_t player)
{
    ...
    /* 初始化动态信号量 */
    sem_init(&player->sem_play, 0);

    /* 创建动态线程 */
    player->play_thread = task_create(player_entry, player, PRIO_NORMAL, "player");
